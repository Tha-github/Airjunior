...
import { button, icon, card } from '../components/ui.mjs';
...
const prepared = '<p class="preparation-note">Conteúdo institucional em preparação.</p>';

export const pages = [
  {
    path: '/', title: 'AAFJ Advocacia | Advocacia em Porto Alegre',
    description: 'Atendimento próximo, orientação clara e estratégia personalizada. AAFJ Advocacia em Porto Alegre. Air Alves Freitas Júnior, OAB/RS 93.942.',
  ...
  { path: '/areas-de-atuacao/', title: 'Áreas de Atuação | AAFJ Advocacia', description: 'Conheça as áreas de atuação da AAFJ Advocacia em Porto Alegre.', content: intro('ÁREAS DE ATUAÇÃO', 'Orientação para cada questão.', `<p class="lead">Conheça nossas áreas de atuação.</p><div class="cards-grid">${areas.map((area, index) => card(area.name, area.href, index + 1)).join('')}</div>`) },
  ...areas.map(area => ({ path: area.href, title: `${area.name} | AAFJ Advocacia`, description: `${area.name} na AAFJ Advocacia, em Porto Alegre. Entre em contato com nossa equipe.`, content: intro('ÁREAS DE ATUAÇÃO', area.name, `${prepared}<div class="intro-actions">${button('Fale com nossa equipe', site.whatsapp)}<a class="text-link" href="/areas-de-atuacao/">Todas as áreas ${icon('arrow')}</a></div>`), noindex: true })),
  { path: '/contato/', title: 'Contato | AAFJ Advocacia em Porto Alegre', description: 'Fale com a AAFJ Advocacia pelo telefone (51) 99386-9229 ou visite o escritório no bairro Sarandi, Porto Alegre.', content: contact({ standalone: true }) },
